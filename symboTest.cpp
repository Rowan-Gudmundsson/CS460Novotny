	...
	while (input != "quit") {
		...
		if (input == "{") {
			...
			if (symbolPtr != nullptr && symbolPtr->scopeLevel == table.scopeLevel) {
				std::cout << "ERROR: Variable " << input << " was already declared on line "
				          << symbolPtr->lineNumber << std::endl;
			} else {
				if (symbolPtr != nullptr && symbolPtr->scopeLevel < table.scopeLevel) {
					std::cout << "WARNING: Variable " << input
					          << " shadowing variable declared on line " << symbolPtr->lineNumber
					          << std::endl;
				}
